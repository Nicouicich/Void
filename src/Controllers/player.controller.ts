...
export class PlayerController {
    ...
    async getPlayer(@Body() summoner: SummonerDto) {
        const summonerID = await this.PlayerService.getPlayerIdByName(summoner.summonerName, summoner.region);
        if (typeof summonerID === 'string') {
            const stats = await this.PlayerService.getPlayerStats(summonerID, summoner.region);
            return stats;
        } else {
            return {
