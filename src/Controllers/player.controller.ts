...
export class PlayerController {
  ...
  @ApiResponse({
    ...
    type: SummonerStatsEntity,
  })
  @ApiResponse({
    status: 400,
    description: 'For example: Player:ET Murtgrafa or region: la4',
  })
  @ApiBody({ type: SummonerDto })
  @ApiProduces('application/json')
  async getPlayer(@Res() res, @Body() summoner: SummonerDto) {
    try {
      logger.log(
        'info',
        `New call at the endpoint /player/ with body:${summoner}`,
      );
      ...
      if (account) {
        const leagues = await Promise.all(
          account.leagues.map(async (league) => {
            league.AverageVisionScore =
              await this.RecentMatchesService.getPlayerVisionScoreDB(
                account.puuid,
                league.queueType,
              );
            league.AverageCSPerMinute =
              await this.RecentMatchesService.getPlayerCSPerMinuteDB(
                account.puuid,
                league.queueType,
              );
            league.AverageKDA = await this.RecentMatchesService.getPlayerKDADB(
              account.puuid,
              league.queueType,
            );
            return league;
        ...
        const accountPlayer = await this.createPlayer(
          summoner.summonerName,
          summoner.region,
        );
        if (accountPlayer) {
          const summonerDto = new SummonerStatsDto(
            summoner.summonerName,
            summoner.region,
            accountPlayer.profileIconId,
            accountPlayer.summonerLevel,
            accountPlayer.leagues,
          );
  ...
  @ApiResponse({
    status: 200,
    description: 'Recent matches from the player filtered by queueId',
    type: SummonerStatsEntity,
  })
  @ApiResponse({
    status: 400,
    description:
      'queueId!= 420, queueId != 440, non-existent region or non-existent user',
  })
  @ApiQuery({ name: 'queueId', description: 'Higher than 1' })
  @ApiProduces('application/json')
  async getPlayerByQueueId(
    @Res() res,
    ...
    try {
      logger.log(
        'info',
        `New call at the endpoint /player/:queueId with params:
      ${queueId},
      and body: ${summoner}`,
      );
      ...
      let account = await this.PlayerService.getPlayerAccountDB(
        summoner.summonerName,
        summoner.region,
      );

      if (!account) {
        account = await this.createPlayer(
          summoner.summonerName,
          summoner.region,
        );
        if (!account) {
      ...
      const matches = await this.RecentMatchesService.getPlayerMatchesDB(
        account.puuid,
        queueId,
      );
      ...
      account.leagues = await this.getPlayerLeague(
        account.puuid,
        queueId,
        summoner.summonerName,
        summoner.region,
  ...
  private getPlayerLeague = async (
    puuid: string,
    queueId: number,
    summonerName: string,
    region: string,
  ): Promise<ISummonerLeague[]> => {
    const matches = await this.RecentMatchesService.getPlayerMatchesDB(
      puuid,
      queueId,
    );
    const queueName = getQueueName(queueId);
    const summonerLeagues: ISummonerLeague = {
      rank: 'Not ranked',
      tier: 'Not ranked',
      queueType: 'all',
      wins: 0,
    ...
    if (queueId == 420 || queueId == 440) {
      const player = await this.PlayerService.getPlayerAccountDB(
        summonerName,
        region,
      );

      const league = player.leagues.filter(
        (element) => element.queueType == queueName,
      );
    ...
    matches.forEach((match) => {
      const data = match.participants.filter(
    ...
    summonerLeagues.AverageKDA = Math.trunc(
      summonerLeagues.AverageKDA / matches.length,
    );
    summonerLeagues.AverageVisionScore = Math.trunc(
      summonerLeagues.AverageVisionScore / matches.length,
    );
    summonerLeagues.winrate =
      (summonerLeagues.wins / (summonerLeagues.wins + summonerLeagues.losses)) *
      100;
  ...
  private getRecentMatchesInfo = async (
    ...
    try {
      ...
      const matchesInfo = recentMatches.map(async (matchID: string) => {
        return await this.RecentMatchesService.getInfoAboutAMatch(
          matchID,
          regionName,
        );
      });
  ...
  private createPlayer = async (summonerName: string, region: string) => {
    try {
      ...
      if (accountPlayer) {
        const regionName = getRegionName(region);
        let leagues: ISummonerLeague[] =
          await this.PlayerService.getPlayerStats(accountPlayer.id, region);
        ...
        recentMatches.length > 0
          ? await this.getRecentMatchesInfo(recentMatches, region)
          : null;
        if (leagues.length) {
          leagues = await Promise.all(
            leagues.map(async (league) => {
              ...
              league.AverageKDA =
                await this.RecentMatchesService.getPlayerKDADB(
                  accountPlayer.puuid,
                  league.queueType,
                );
              return league;
