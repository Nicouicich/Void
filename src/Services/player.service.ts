...
export class PlayerService {
  ...
  getPlayerAccountDB(
    name: string,
    region: string,
  ): Promise<SummonerStatsEntity> {
    const response = this.playerRepo.findOneBy({
      summonerName: name,
      region: region,
    });
    return response;
      ...
      if (status === this.STATUS_CORRECT) {
        ...
        if (summonerLeagues.length == 0) {
          ...
          const sumLeagues = summonerLeagues.map((league) => {
            ...
            const summonerLeague: ISummonerLeague = {
              tier: league.tier,
              rank: league.rank,
  ...
  public async getPlayerAccount(summonerName: string, region: string) {
    try {
      ...
      logger.log(
        'error',
        `Error while requesting an account player by name. Probably cause: ${e}
            Check API Key`,
      );
      return null;
    }
