...
export class RecentMatchesService {
  ...
  public async getRecentMatchesByQueueId(
    queueId: number,
    puuid: string,
    region: string,
    recentMatches = 20,
  ...
  public async getInfoAboutAMatch(
    matchID: string,
    region: string,
  ): Promise<MatchEntity> {
    try {
      ...
      if (response) {
        ...
        const participants: IMatchParticipant[] =
          response.data.info.participants.map((participantInfo) => {
            const kda = participantInfo.deaths
              ? Math.trunc(
                  (participantInfo.kills + participantInfo.assists) /
                    participantInfo.deaths,
                )
              : participantInfo.kills + participantInfo.assists;
            const participant: IMatchParticipant = {
              ...
              csPerMinute:
                participantInfo.totalMinionsKilled / (gameDuration / 60),
        ...
        const match: MatchEntity = new MatchEntity(
          ...
          gameDuration,
          participants,
          response.data.metadata.participants,
        );
        return match;
      }
    } catch (e) {
